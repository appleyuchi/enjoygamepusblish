...
function change_goods_num(key, idstr, sn, param_id, zk) {
    ...
    if (checkRate(num) === false) {
        $.post('/self_define/ajax_set_info.php', {
            type: 30,
            id: idstr,
            sn: sn,
            username: user_name,
            userid: ev_log_userid,
            std: std
        }, function(data) {
            if (data.indexOf('|') == -1) {
                return false;
            ...
            if (gouwuche) {
                ...
                for (var key in aOrder) {
                    if (key == (proId + '_' + guigeId)) {
                        aOrder[key] = {
                            'num': parseInt(aOrder[key].num) + buyCnt,
                            'sort': aOrder[key].sort
                        };
