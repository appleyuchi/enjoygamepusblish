if (!$.browser.msie || $.browser.msie && $.browser.version * 1 > 10) {
            ...
            var a = this,
                c = function() {
                    ...
                    for (a.ctx.clearRect(0, 0, a.canvas.width, a.canvas.height), d = 0; d < a.flakeCount; d++) {
                        if (e = a.flakes[d], f = a.mx, g = a.my, h = 100, i = e.x, j = e.y, k = Math.sqrt((i - f) * (i - f) + (j - g) * (j - g)), h > k) l = h / (k * k), m = (f - i) / k, n = (g - j) / k, o = l / 2, e.velX -= o * m, e.velY -= o * n;
                        else switch (e.velX *= .98, e.velY <= e.speed && (e.velY = e.speed), a.settings.windPower) {
                            case !1:
    ...
    (function(a) {
        ...
        o.prototype = {
            ...
            getPos: function(a) {
                function b() {
                    if (_this.x = Math.random() * d, i.length)
                        for (var c = 0; c < i.length; c++)
                            if (Math.abs(_this.x - i[c]) < j) return b();
                    i[a] = _this.x
                }
    ...
    (function(a) {
        ...
        f.prototype = {
            constructor: f,
            createParticles: function(a, b, d) {
    ...
    (function(a) {
        ...
        b.prototype.init = function() {
            ...
            var c = this,
                d = c.ctx,
                e = c.ctx.createLinearGradient(a, b, a - c.dir, b + c.height);
                ...
                case 'wechat':
                    ...
                    if (src) {
                        ...
                        img.onload = function() {
                            wsf.f.tip.addTip({
                                getDom: $this.closest('li'),
                                html: '<div class="twoCode"><div class="code-img"><img src="' + src + '" /></div></div>'
                            });
                        };
                        img.src = src;
                    }
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    if (data.success) {
                                        ...
                                        wsf.pDom.htmlDom.animate({
                                            scrollTop: Math.min(wsf.pDom.htmlDom.scrollTop(), t.closest('.customModuleRow').offset().top)
                                        });
                                        wsf.pDom.bodyDom.animate({
                                            scrollTop: Math.min(wsf.pDom.bodyDom.scrollTop(), t.closest('.customModuleRow').offset().top)
                                        });
        ...
        data.success * 1 && ((data.list && data.list.length) || (data.sysList && data.sysList.length) || (data.docSysList && data.docSysList.length)) && (function() {
            ...
            $.each(docSysList, function(i, v) {
                ...
                if (curSocial.length) {
                    var collectCnt = v.collectCnt * 1,
                        likeCnt = v.likeCnt * 1,
                        readCount = v.readCount * 1,
                        replyCount = v.replyCount * 1;
                ...
                click: function(ev) {
                    ...
                    switch (true) {
                        ...
                        case targetDom.hasClass('w-l-e'):
                            ...
                            $this.attr({
                                'data-type': wle.attr('data-type'),
                                'data-url': wle.attr('data-url')
                            });
                            break;
